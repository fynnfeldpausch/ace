...
object CypherParser {
  ...
  def flatten[T1, T2, R](implicit f: org.neo4j.ace.TupleFlattener[(T1 ~ T2) => R]): ((T1 ~ T2) => R) = f.f

  def str(columnName: String): RowParser[String] = get[String](columnName)(implicitly[org.neo4j.ace.Column[String]])
  ...
  def get[T](columnName: String)(implicit extractor: org.neo4j.ace.Column[T]): RowParser[T] = RowParser { row =>
    import MayErr._
